...
(function ($) {
    ...
    $.fn.imageUploader = function (options) {
        ...
        let defaults = {
            ...
            extensions: ['.jpg', '.jpeg', '.png', '.JPG', '.JPEG', '.webp'],
        ...
        let validateExtension = function (file) {
            ...
            if (plugin.settings.extensions.indexOf(file.name.replace(new RegExp('^.*\\.'), '.')) < 0) {
                ...
                var error_msg = `The file "${file.name}" does not match with the accepted file extensions: "${plugin.settings.extensions.join('", "')}"`;
        ...
        let validateMIME = function (file) {
            ...
            if (plugin.settings.mimes.indexOf(file.type) < 0) {
                ...
                var error_msg = `The file "${file.name}" does not match with the accepted mime types: "${plugin.settings.mimes.join('", "')}"`;
        ...
        let validateMaxSize = function (file) {
            ...
            if (file.size > plugin.settings.maxSize) {
                var error_msg = `The file "${file.name}" exceeds the maximum size of ${plugin.settings.maxSize / 1024 / 1024}Mb`;
        ...
        let validateMaxFiles = function (index, file) {
            ...
            if ((index + dataTransfer.items.length + plugin.settings.preloaded.length) >= plugin.settings.maxFiles) {
                ...
                var error_msg = `The file "${file.name}" could not be added because the limit of ${plugin.settings.maxFiles} files was reached`;
